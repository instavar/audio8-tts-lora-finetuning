...
Capability schema 1.2 separates validated CUDA evidence from bounded MPS evidence and leaves adapter-aware ONNX and SGLang paths unverified. The current bounded CUDA lifecycle retains one selected adapter, completes every objective metric required by its two-row frozen plan, publishes a content-addressed package, and reproduces one frozen waveform in two new same-host restore directories. This closes the prior local artifact-retention blocker for that exact run. It does not establish a matched Base improvement, blind perceptual quality, clean-host portability, independent backup, disaster recovery, or cross-runtime equivalence.
